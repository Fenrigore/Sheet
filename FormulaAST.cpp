    ...
    namespace {
        ...
        public:
            ...
            double Evaluate(const std::function<double(Position)>& args) const override {
                ...
                if (type_ == Add) return left + right;
                if (type_ == Subtract) return left - right;
                if (type_ == Multiply) return left * right;
                if (type_ == Divide) {
                    double res = left / right;
                    // Проверка на деление на ноль (в том числе бесконечность)
                    if (!std::isfinite(res)) throw FormulaError(FormulaError::Category::Arithmetic);
                    return res;
                }
        ...
        public:
            ...
            double Evaluate(const std::function<double(Position)>& args) const override {
                ...
                return type_ == UnaryPlus ? val : -val;
            }
...
FormulaAST::FormulaAST(std::unique_ptr<ASTImpl::Expr> root_expr, std::forward_list<Position> cells)
    ...
    cells_.unique();//удаляем дубликаты
}
